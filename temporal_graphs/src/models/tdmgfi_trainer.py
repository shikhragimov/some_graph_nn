...
class DMGITrainer:
    ...
    def train_epoch(
        ...
        loss_rel, loss_cr, loss = self.model.loss(pos_hs, neg_hs, summaries)
        loss.backward()
        optimizer.step()

        return float(loss_rel), float(loss_cr),  float(loss)
    ...
    def train(
        ...
        for epoch in range(1, epochs + 1):
            loss_rel, loss_cr, loss = self.train_epoch(optimizer=optimizer)
            if epoch % print_every_n_epoch == 1:
                LOGGER.info(
                    f"Epoch: {epoch:03d}, "
                    f"Loss: {loss:.3f}, "
                    f"loss rel: {float(loss_rel):.3f}, "
                    f"loss_cr : {float(loss_cr):.3f}")
